	...
	function doforgot_password(){
		if(IS_POST){
				...
				$rules = array(
						...
						array('mobile', 'require', '手机号不能为空！', 1 ) // 验证手机字段格式是否正确
				);
				...
				if($users_model->validate($rules)->create()===false){
					...
					if($find_user){
						...
						if($flag!=1)
						{
							$this->success('验证成功！',U("user/login/password_reset",array('hash'=>$activekey)));
						//$this->assign('id',$find_user['id']);
						//$this->display(":password_reset");
						}
						else
						{	
							$this->success('验证成功！',U("portal/mindex/setnewpassword",array('hash'=>$activekey)));
						}
					}else {
						$this->error("该手机号不存在！");
					}
...
hello;
	...
	function dopassword_reset(){
		if(IS_POST){
			...
			$rules = array(
					...
					//array('password','length','密码长度最小5位,最大20位！',1,'5,20'),
			...
			if($users_model->validate($rules)->create()===false){
				...
				$result=$users_model->where(array("id"=>$id))->save(array("user_pass"=>$password));
				if($result){
	...
	function dopassword_reset(){
		if(IS_POST){
			...
			$rules = array(
					...
					array('password', 'require', '密码不能为空！', 1 ),
					//array('password','length','密码长度最小5位,最大20位！',1,'5,20'),
					array('password','5,20','密码长度最小5位,最大20位！',3,'length'),
					array('repassword', 'require', '重复密码不能为空！', 1 ),
					array('repassword','password','确认密码不正确',0,'confirm'),
					array('hash', 'require', '重复密码激活码不能空！', 1 ),
			);
	...
	*/
		...
		$login_type = I('post.login_type',0,'intval');
		//if($login_type==0){$rules[]=array('password','require','密码不能为空！',1);}
    	if($users_model->validate($rules)->create()===false){
    		$this->error($users_model->getError());
    	}
    ...
    private function _do_mobile_login(){
        ...
        $login_type = I('post.login_type',0,'intval');
		if($login_type==1){$password =  $result['user_pass'];}
        ...
        if(!empty($result)){	
            if(sp_compare_password($password, $result['user_pass'])||$login_type==1){
                $_SESSION["user"]=$result;
		//setcookie("user_info",json_encode($result,true),time()+30*24*3600,"/");
		//cookie('userInfo',json_encode($result,true),array('expire'=>time()+30*24*3600,'prefix'=>'tyhd_'));
		...
		session('user',$result,604800);
                ...
                $_SESSION['login_http_referer']="";
				 //setcookie("username",$remember,time()+3600*24*365);
				if($remember==1){
				   setcookie("username",$result['mobile'],time()+3600*24*365);
				   setcookie("password",$password,time()+3600*24*365);
				}
				if($remember==0)
				{
					//setcookie("username",$result['mobile'],time()-3600);
