...
class RegisterController extends HomebaseController {
	...
	private function _do_mobile_register(){
        ...
        $rules = array(
			...
			array('vertify', 'require', '验证码不能为空！', 1 ),
	    ...
	    if($result){
	        ...
	        $data=array(
	            ...
	            'user_status' => 1,
	            "user_type"=>2,//会员
				//"level"=>I('post.level',0,'intval'),//激活码
				"level"=>0,

	        );
			$level = trim(I('post.level',''));
			//分析激活码的有效性
			$regcode = D('Regcode')->where(array('code'=>$level,'status'=>0))->find();
			
			if($regcode){
				//D('Regcode')->where(array('id'=>$regcode['id']))->save(array('status'=>0));
				//记录用户使用的激活码。
				$data['level'] = $regcode['type'];
				$data['valid_time'] = strtotime('1 years');
				//更改激活码的状态。
				D('Regcode')->where('id='.$regcode['id'])->save(array('status'=>1));
			}else{
				if($level=='天地人和'){
					$data['level'] = 2;
					$data['permission'] ="shakeprize,envelope,nenvelope";
					$data['valid_time'] = strtotime('1 months');
					$data['invite_code'] = "天地人和";
				}else{
					$data['valid_time'] = strtotime('1 months');
				}
			}
	        ...
	        if($rst){
	            ...
	            $data['id']=$rst;
	            $_SESSION['user']=$data;
	            $this->success("注册成功！".$regcode['id'],__ROOT__."/");
	        }else{
				$this->error($users_model->getError());
