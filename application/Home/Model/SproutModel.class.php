...
class SproutModel extends Model{
    ...
    public function rankList($parm){
        $data = M('sign')
                ->join('LEFT JOIN '.C('DB_PREFIX').'customer ON '.C('DB_PREFIX').'customer.user_id = '.C('DB_PREFIX').'sign.user_id')
   ...
   //投票
    public function toVote($parm){
        ...
        return ['msg'=>$user_info['openid'],'status'=>false];
        ...
        if ($res['vote_start_at']) {
            ...
            if ($res['limit_num']) {
                ...
                $hoursWhere = ['user_id'=>$user_info['openid'],'sign_id'=>$parm['sign_id'],'create_at'=> array(array(array('egt',$start_at),array('elt',$end_at)),'AND')];
        ...
        if ($res['voting_form']==1) {
            ...
            $countWhere = ['user_id'=>$user_info['openid'],'create_at'=> array(array(array('egt',$start_at),array('elt',$end_at)),'AND')];
        }else{
            $countWhere = ['user_id'=>$user_info['openid']];
        }
        //判断每天最大投票数
