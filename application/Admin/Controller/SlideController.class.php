...
class SlideController extends AdminbaseController{
	...
	function add_post(){
		if(IS_POST){
			...
			if ($this->slide_model->create()) {
				...
				$_POST['slide_index']=sp_asset_relative_url($_POST['slide_index']);
				$post = $_POST;
				if ($this->slide_model->add($post)!==false) {
	...
	function edit()
	{
		...
		$this->assign($slide);
		$this->assign("smeta", json_decode($slide['smeta'],true));
		$this->display();
	}
	...
	function edit_post()
	{
		if (IS_POST) {
			...
			if ($this->slide_model->create()) {
				...
				$_POST['slide_index'] = sp_asset_relative_url($_POST['slide_index']);
				$post = $_POST;
				if ($this->slide_model->where(['slide_id' => I("post.slide_id",0,"intval")])->save($post)!==false) {
